											...
											EnemyMissileTypes, BossTypes
{
	...
	{		
		...
		else if(level == 20)
		{
			...
			selection = 0;
			helicopter.powerUp_decay();
			if((helicopter.no_cheats_used||Events.save_anyway) && !Events.reached_level_20[helicopter.type])
			{
	...
	public static void generate_new_enemies( ArrayList<LinkedList<Enemy>> enemy, Helicopter helicopter)
	{
	...
	{		
		...
		return !creation_stop 
				&&((Events.last_creation_timer > 20  && !Events.is_boss_level()) || 
				   (Events.last_creation_timer > 135 &&  Events.is_boss_level()) ) 
	                              ...
	                              Helicopter helicopter)
	{
		...
		for(Iterator<Enemy> i = hd.enemy.get(ACTIVE).iterator(); i.hasNext();)
		{
			...
			{								
				enemy.update(hd, helicopter);				
			}
			else if(enemy.is_destroyed)
			{
				i.remove(); 
				hd.enemy.get(DESTROYED).add(enemy);
			}			
			else
			{
				enemy.clear_image();
				i.remove(); 
				hd.enemy.get(INACTIVE).add(enemy);
			}			
		}
	}	
	...
	{												
		this.lifetime++;		
		this.update_timer();	
		if(this.call_back > 0 && Events.is_boss_level()){this.call_back = 0;}		
		if(current_rock != this){check_for_barrier_collision();}		
		if(this.stunning_timer == READY)
		{
			this.update_stoppable_timer();
			if(this.is_major_boss()){this.calculate_boss_maneuver(hd.enemy);}
			this.calculate_flight_maneuver(hd, helicopter);
			this.validate_turns();
		}		
	...
	{				
		...
		// Shooting
		if(this.shoot_timer != DISABLED){evaluateShooting(hd, helicopter);}
		if(this.barrier_shoot_timer != DISABLED)
		{
			evaluateBarrierShooting(hd, helicopter);
		}									
	...
	*/
	...
	private void emp_shock(HelicopterDefence hd, Helicopter helicopter)
    {
		...
		{			
			...
			Explosion.start(hd.explosion, helicopter, 
							this.bounds.getCenterX(), 
		...
		else
		{
			...
			this.die(hd, helicopter, null, false);
		}
    }
		...
		if(	this.shoot_timer == 0 
			...
			&& ((this.direction.x == -1 
				 && helicopter.bounds.intersects(	
						 				...
						 				this.bounds.getY() + (this.model == 0 ? 0 : this.bounds.getWidth()/2) - 15, 
						 				Integer.MAX_VALUE/2, 
	...
	private void move()
	{			
		if(!this.speed.equals(ZERO_SPEED)||BgObject.background_moves)
		{
			this.set_location(	
					this.bounds.getX() 
					...
					Math.max( this.model == BARRIER ? 0 : Integer.MIN_VALUE,
							current_rock == this ? this.bounds.getY() :
	                                 ...
	                                 Helicopter helicopter)
	{
		...
		{
			Enemy e = i.next();
			e.update_dead(hd.explosion, helicopter);				
			
			if(	helicopter.basic_collision_requirements_satisfied(e) 
				&& !e.has_crashed)
			{
				e.collision(hd, helicopter);
			}				
	...
	public void react_to_radiation(HelicopterDefence hd, Helicopter helicopter)
	{
		...
		else if(this.can_take_collison_dmg())
		{
			...
			else
			{
				...
				this.die(hd, helicopter, null, beam_kill);					
			}
		}		
	}
	...
	{		
		...
		if(missile == null)
		{
			this.explode(hd.explosion, helicopter);
		}		
		else if(missile.type != STUNNING)
		{
			this.explode(hd.explosion, helicopter, missile);
		}		
